...
const DEFAULT_URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${API_GEMINI}`;
const FALLBACK_URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${API_GEMINI}`;
...
async function getOrCreateSheet(token) {
    ...
    try {
        ...
        const buscaRes = await fetch(`https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(query)}`, {
        ...
        showToast("info", "Planilha não encontrada. Criando uma nova com a aba correta...");
        
        const criaRes = await fetch(`https://sheets.googleapis.com/v4/spreadsheets`, {
            method: "POST",
            headers: {
...
async function getDataFromExcel() {
    try {
        const res = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${ABA}`, {
            headers: { Authorization: `Bearer ${token}` },
        });
...
async function insertSheetItem(data, isEdit = false) {
    try {
        if (window.insertingData) return;
        window.insertingData = true;
        ...
        if (isEdit && data[0] !== undefined) {
            ...
            const indexLinha = excelData.findIndex((row) => Number(row[0]) === Number(data[0])) + 1;
        ...
        window.insertingData = false;
    }
}

async function removeSheetItem(id) {
    try {
        if (window.removingData) return;
        window.removingData = true;

        const rawData = await getDataFromExcel();
        const baseData = rawData?.values || [];
        const linhasFiltradas = baseData.slice(1).filter(row => Number(row[0]) !== Number(id));
        ...
        showToast("error", `Não foi possível remover o item #${id} da planilha! ${error}`);
    } finally {
        window.removingData = false;
    }
}

async function clearSheet() {
    try {
        const res = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${ABA}:clear`, {
            method: "POST",
            headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json",
...
async function getFormatedData(filterType) {
    ...
    const baseData = (allData.values && allData.values.length > 0) ? allData.values : [];
    const formatedData = [...baseData].formatItensToMap();
    ...
    google.charts.setOnLoadCallback(async () => {
        const datatable = (data, chartType) => {
            ...
            return google.visualization.data.group(
                dt,
                [{ column: 0, modifier: (d) => d, type: chartType === "pie" ? "string" : "date" }],
...
async function requestAI(url, payload) {
    for (let i = 0; i <= AI_MAX_RETRIES; i++) {
        ...
        try {
            ...
            if (!response.ok) {
                console.log(`Aguardando ${waitTime}ms para tentar novamente a URL "${url}"...`);
            ...
            throw new Error(`Ocorreu um erro inesperado para a url "${url}".`);
            if (i === AI_MAX_RETRIES) throw new Error(`Falharam todas as tentativas para a url "${url}".`);
            await sleep(waitTime);
        }
    }
}
...
async function consultAI(promptType, context = "") {
    if (Array.isArray(context)) context.join(",");
    ...
    if (!resposta) {
        console.warn("API para Gemini Flash 2.5 falhou, tentando com a API do Gemini Flash 2.0...");
...
Array.prototype.formatItensToMap = function() {
    const fieldMaps = [
        "id",
        "descricao",
        "dt_criacao",
...
Array.prototype.formatItensToArray = function() {
...
function showToast(type = "success", customText) {
    ...
    if (type.toLowerCase() === "success") {
        ...
        toastText = !toastText ? "Não foi possível realizar a operação!" : toastText;
...
function loadTheme() {
    ...
    const switchElement = document.querySelector(".theme-switch")?.querySelector(".switch");
...
function showDataForm(formData) {
    ...
    if (formData) {
        fetch(window.location.href, {
            method: "POST",
            body: formData,
        }).catch(error => console.error("Erro ao passar dados ao formulário! ", error));
    }
}
...
function loadTableData() {
    ...
    data.map((row) => {
        content += `<tr class="table-row" data-id="${row.id}">
            <td class="table-description">${row.descricao}</td>
            <td class="table-value">${isNaN(row.valor) ? row.valor : "R$ " + String(Number(row.valor).toFixed(2))}</td>
            <td class="table-portion">${row.num_parcela} / ${row.qtd_parcelas}</td>
            <td class="table-category">${row.categoria}</td>
            <td class="table-method"><i class="${metodoPagIcon[row.met_pagamento]}" title="${row.met_pagamento}"></i></td>
            <td class="table-actions"><a href="javascript:removeSheetItem(${row.id})" style="text-decoration: none; color: var(--text-color)"><i class="fa fa-fw fa-solid fa-trash red" title="Deletar"></i></a></td>
        </tr>`;
    });

    tbody.innerHTML = content;

    document.querySelector("table#tabela-dados")?.addEventListener("click", (e) => {
        ...
        formData.append("descricao", element.querySelector(".table-description")?.textContent.trim());
        formData.append("categoria", element.querySelector(".table-category")?.textContent.trim());
        formData.append("valor", element.querySelector(".table-value")?.textContent.trim().replace("R$ ", ""));
        formData.append("met_pagamento", element.querySelector(".table-method i")?.title.trim());

        const [part1, part2] = element.querySelector(".table-portion")?.textContent.trim().split("/");
